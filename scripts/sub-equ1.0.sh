...
function run_minimize {
  ...
  ibrun -n 1 -o $iproc /usr/local/bin/task_affinity singularity exec --bind $scratchdir:$dir2 --pwd $dir2 --nv $SIF_EXEC /home/tinker/bin/dynamic.gmix $simname $nsteps $dt $svfreq $ensemble $temp $pressure -k tinker0.key &> $scratchdir/min1.log 
	fi
}
